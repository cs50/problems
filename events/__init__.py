...
@check50.check(valid)
def two_events(project):
    """project contains at least two events"""

    num_events = parse_blocks(project)["events"]

    if num_events < 2:
        raise check50.Failure(f"only {num_events} event{'' if num_events == 1 else 's'} found, at least 2 required")
