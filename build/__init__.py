...
@check50.check(valid)
def three_backdrops(project):
    """project contains at least three backdrops"""
...
@check50.check(valid)
def one_broadcast(project):
    """project contains at least one broadcast"""

    num_broadcasts = parse_blocks(project)["broadcasts"]

    if num_broadcasts < 1:
        raise check50.Failure("at least one broadcast required")


@check50.check(valid)
def one_clone(project):
    """project contains at least one clone"""

    num_clones = parse_blocks(project)["clones"]

    if num_clones < 1:
        raise check50.Failure("at least one clone required")


@check50.check(valid)
def one_custom(project):
    """project contains at least one custom block"""

    num_customs = parse_blocks(project)["customs"]
...
@check50.check(valid)
def one_variable(project):
    """project contains at least one variable"""

    num_variables = parse_blocks(project)["variables"]

    if num_variables < 1:
        raise check50.Failure("at least one variable required")


@check50.check(valid)
def one_loop(project):
    """project contains at least one loop"""

    num_loops = parse_blocks(project)["loops"]

    if num_loops < 1:
        raise check50.Failure("at least one loop required")


@check50.check(valid)
def one_condition(project):
    """project contains at least one condition"""

    num_conditions = parse_blocks(project)["conditions"]

    if num_conditions < 1:
        raise check50.Failure("at least one condition required")


@check50.check(valid)
def one_event(project):
    """project contains at least one event"""

    num_events = parse_blocks(project)["events"]

    if num_events < 1:
        raise check50.Failure("at least one event required")
