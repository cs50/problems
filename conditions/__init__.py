...
@check50.check(valid)
def one_if(project):
    """project contains at least one if block"""

    num_ifs = parse_blocks(project)["ifs"]

    if num_ifs < 1:
        raise check50.Failure("at least one if block required")


@check50.check(valid)
def one_ifelse(project):
    """project contains at least one if else block"""

    num_ifelses = parse_blocks(project)["ifelses"]

    if num_ifelses < 1:
        raise check50.Failure("at least one if else block required")
