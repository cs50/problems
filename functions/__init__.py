...
@check50.check(valid)
def one_motion(project):
    """project contains at least one motion block"""

    num_motions = parse_blocks(project)["motions"]

    if num_motions < 1:
        raise check50.Failure("at least 1 motion block required")


@check50.check(valid)
def one_looks(project):
    """project contains at least one looks block"""

    num_looks = parse_blocks(project)["looks"]

    if num_looks < 1:
        raise check50.Failure("at least 1 looks block required")


@check50.check(valid)
def one_sound(project):
    """project contains at least one sound block"""

    num_sounds = parse_blocks(project)["sounds"]

    if num_sounds < 1:
        raise check50.Failure("at least 1 sound block required")
