...
def test_terminate():
    ...
    check50.run("python3 coke.py").stdin(input, prompt=True).stdin(input, prompt=True).stdin(input, prompt=True).stdin(input, prompt=True).stdin(input, prompt=True).stdout(case_insensitive_prefix + output, output, regex=True).exit()
...
def test_change():
    ...
    check50.run("python3 coke.py").stdin(input, prompt=True).stdin("10", prompt=True).stdin(input, prompt=True).stdout(case_insensitive_prefix + output, output, regex=True).exit()
