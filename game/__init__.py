...
def test_too_large():
    ...
    output = "Too large!"
    check50.run("python3 game.py").stdin("5", prompt=True).stdin("10", prompt=True).stdout(regex(output), output, regex=True).reject()
...
def test_just_right():
    ...
    output = "Just right!"
    check50.run("python3 game.py").stdin("1", prompt=True).stdin("1", prompt=True).stdout(regex(output), output, regex=True).exit()
...
def test_too_small():
    ...
    output = "Too small!"
    check50.run("python3 game.py").stdin("1000000000", prompt=True).stdin("1", prompt=True).stdout(regex(output), output, regex=True).reject()
