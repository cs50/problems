...
def test_too_large():
    ...
    output = "Too large!"
    check50.run("python3 testing.py").stdin("5", prompt=True).stdin("6", prompt=True).stdout(regex(output), output, regex=True).reject()
...
def test_just_right():
    ...
    output = "Just right!"
    check50.run("python3 testing.py").stdin("5", prompt=True).stdin("5", prompt=True).stdout(regex(output), output, regex=True).exit(0)
